...
// Note: Manager is a type alias (type Manager = *actionManagerMap),
// not an interface. Keep service interfaces focused and small.
```
...
func TestMyPlugin(t *testing.T) {
    ...
    assert.Equal(t, 100, info.Weight)
...
### Avoid Panics
```go
// Current implementation panics on duplicate registration.
// Consider returning error in future:
...
// Current: panics on duplicate (acceptable for init()-time registration)
