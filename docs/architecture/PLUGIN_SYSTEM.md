...
type PluginInfo struct {
    ...
    pkgPath  string  // Set automatically via InitPluginInfo()
    typeName string  // Set automatically via InitPluginInfo()
}
```
...
4. **DiscoveryPlugin** - Action discovery hooks (defined in `pkg/action/discover.go`)
```go
...
Plugins can access application services through dependency injection via `ServiceManager`:
...
4. **Panic on conflicts**: Duplicate plugin registrations cause panics
