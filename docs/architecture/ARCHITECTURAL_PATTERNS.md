...
### 4. **Large Type Surface** ⚠️
**Problem**: `Manager` (type alias for `*actionManagerMap`) has 19+ exported methods
**Impact**: Large API surface, though not an interface issue since `Manager` is a concrete type alias
...
**Note**: `Manager` is currently a type alias (`type Manager = *actionManagerMap`), not an interface.
Introducing focused interfaces would enable better testing and decoupling:
```go
